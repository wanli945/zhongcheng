        ...
        <el-tab-pane name="first" label="单个导入">
            <div class="editBox" v-if="activeName==='first'">
                <el-form size="small" style="height: 440px" :model="addDialog.data"
                    ...
                    <el-form-item label="现有套餐类型" prop="nowPackageType">
                        <el-input v-model.trim="addDialog.data.nowPackageType"
                                  autocomplete="on"></el-input>
                    </el-form-item>
                    <el-form-item label="现有优惠" prop="nowDiscount">
                        <el-input v-model.trim="addDialog.data.nowDiscount"
                                  autocomplete="on"></el-input>
                    </el-form-item>
                    ...
                    <el-form-item label="地区" prop="c4name">
                        <el-input v-model.trim="addDialog.data.c4name"
                                  autocomplete="on"></el-input>
                    </el-form-item>
                    ...
                    <!--<el-form-item label="渠道"  prop="channel">
                        <el-select v-model.trim="addDialog.data.channel" placeholder="请选择" clearable>
                            <template v-for="(item,index) in addDialog.option.channel">
                                <el-option
                                    :key="index"
                                    :label="item.label"
                                    :value="item.value">
...
<script>
    ...
    export default {
            ...
            return {
                ...
                addDialog: {
                    data: {
                        ...
                        name: "", // 客户姓名
                        interior: "", // 装机地址
                        channels: businessDefaultChannel.breathe.default, // 来源
                        channel: "",
                        area: "杭州市", //  区域
                    ...
                    rules: {
                        name: [{
                            ...
                            min: 2,
                            message: '用户姓名格式不对', trigger: 'blur',
                        }],
                        interior: {required: true, message: '请输入安装地址', trigger: 'blur'},
                        nowPackageType: {required: true, message: '请输入先有套餐类型', trigger: 'blur'},
                        nowDiscount: {required: true, message: '请输入现有优惠', trigger: 'blur'},
                        c4name: {required: true, min: 2, message: '请输入c4,至少2位', trigger: 'blur'},
                        phone: [
                            {required: true, message: '请输入联系方式', trigger: 'blur'},
                            {
                                /*validator: (rule, value, callback) => {
                                    if (!(this.$reg.contact.test.m(value))) {
                                        return callback(new Error("联系号码格式不对"))
                                    } else {
                                        return callback()
                                    }
                                },*/
                                pattern: this.$reg.phone.digit,
                                message: '联系号码格式不对', trigger: 'blur',
                            },
                        ],
                        broadband: [{required: true, message: '请输入业务号码', trigger: 'blur'}],
                        channels: {required: true, message: '请选择渠道', trigger: 'blur'},
                        channel: {required: true, message: '请选择渠道', trigger: 'blur'},
                        /*nameofadvertiser: {required: true, message: '请输入公司名称', trigger: 'blur'},*/
                        idcard: [
                            {
                                pattern: this.$reg.idCard.all,
                                message: '15或18位身份证号', trigger: 'blur'
                            },
                        ],
                    ...
                    option: {
                        channels: [],
                        advertiserList: businessAllAdvertiser["breathe"]
                    },
                },
        ...
        methods: {
            async init() {
                try {
                    const {dataObj, channelDict} = await this.$root.query_channel(true, businessDefaultChannel.breathe.channelArr)
                    ...
                    this.$refs["addFinance"].validate(async (valid) => {
                        if (valid) {
                            ...
                            this.$axios.post(this.api.insertOneFi, {}, {
                                params: {
                                    ...this.addDialog.data,
                                    userInfo: this.pageRight === 'oneself' && this.userInfo,
