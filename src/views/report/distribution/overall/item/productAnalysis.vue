<!--
 --@IDE      : WebStorm
 --@time     : 2021/2/22 16:19
 --@author   : 李艳鹏
 --@author   : 李艳鹏
 --@Software : app
 --@File     : productAnalysis
 --@describe : 产品分析
-->

<template>
    <my-plat title="产品分析" ref="plat" @choose="chooseChannel" @seleChange="selechange">
        <template #content>
            <el-row :gutter="24">
                <el-col :span="14">
                    <my-report-table :column-arr="columnArr" :table-data="tableData"
                                     :empty-text="emptyText"></my-report-table>
                </el-col>
                <el-col :span="10">
                    <div style="width: 100%;min-width:500px;min-height: 500px">
                        <v-echarts
                            v-if="showCanvas"
                            style="display: inline-block;height: 500px!important;"
                            :options="echertOptions"></v-echarts>
                    </div>
                </el-col>
            </el-row>

        </template>
        <template #right>
            <my-xia-zai></my-xia-zai>
        </template>
        <template #center>
            <el-row :gutter="24" style="width: 50%;margin:0;" type="flex" justify="end">
                <el-col :span="10">
                    <i style="display: inline-block;font-size: 18px">地市：</i>
                    <el-select placeholder="请选择地市" clearable v-model="options.c3name">
                        <template v-for="(item,index) of c3nameList">
                            <el-option :value="item.value" :label="item.label"
                                       :key="index"></el-option>
                        </template>
                    </el-select>
                </el-col>
            </el-row>
        </template>
    </my-plat>
</template>

<script>
    import MyPlat from "../../../components/MyPlat";
    import {echertOptions} from "../data/index"
    import {productAnalysis} from "../data/mock"
    import MyReportTable from "../../../components/myReportTable";
    import MyXiaZai from "../../../components/myXiaZai";

    export default {
        name: "productAnalysis",
        components: {MyXiaZai, MyReportTable, MyPlat},
        data() {
            const columnArr = [
                {label: "产品速率", prop: "productRate"},
                {label: "销售量", prop: "num"},
                {label: "占比", prop: "rate"},
            ]
            return {
                columnArr,
                showCanvas: true,
                options: {
                    disname: "",
                    time: "",
                    timeType: "",
                    city: "",
                    pageCur: 1,
                    c3name: "",
                    dataRange: []
                },
                tableData: [],
                emptyText: "暂无数据",
                total: 0,
                c3nameList: [],
                echertOptions: {
                    title: {
                        name: "产品报表",
                        show: false,
                        top: 'top',
                        left: 'center'
                    },
                    dataset: {
                        source: []
                    },

                    tooltip: Object.assign({}, echertOptions.tooltip, {
                        trigger: "item",
                        axisPointer: { // 坐标轴指示器，坐标轴触发有效
                            "type": "line" // 默认为直线，可选为："line" | "shadow"
                        }
                    }),
                    grid: [
                        {
                            top: 0,
                            bottom: 0,
                            left: 0,
                            containLabel: true
                        },
                    ],
                    series: [
                        {
                            type: 'pie',
                            radius: ['30%', '70%'],
                            right: 0,
                            name: "产品速率",
                            tooltip: {
                                formatter: (rest) => {
                                    const {data, marker, percent} = rest
                                    return `${marker} ${data.productRate}：${data.num}（${percent}%）`
                                }
                            },
                            label: {
                                formatter: "{b}({d}%)"
                            }
                        }
                    ]
                }
            }
        },
        props: {
            statisticsTime: String,
            scope: null,
        },
        methods: {
            async init() {
                this.options.pageCur = 1;
                await this.getProductRank();
            },

            // 全部 按钮事件
            async chooseChannel() {
                this.options.disname = ''
                await this.getProductRank();
            },
            // 下拉改变事件
            async selechange(e) {
                console.log(e)
                this.options.disname = e
                await this.getProductRank();
            },
            async getProductRank() {
                this.options.time = this.scope.options.day;
                this.options.timeType = this.scope.options.statisticsTime;
                this.options.dataRange = this.scope.options.dateRange;
                this.options.city = this.scope.options.city;
                this.tableData = [];
                this.emptyText = "暂无数据"
                this.total = 0;
                this.showCanvas = false
                try {
                    console.log(this.options)
                    const {total, dataArr} = await productAnalysis(this);
                    this.tableData = dataArr;
                    this.total = total || 0;
                    this.echertOptions.dataset.source = dataArr;

                } catch (e) {
                    this.emptyText = e.message
                } finally {
                    this.showCanvas = true
                }
            }
        },
        watch: {
            async "options.disname"() {
                if (this.options.pageCur === 1) {
                    await this.getProductRank();
                } else {
                    this.options.pageCur = 1
                }
            },
            async "options.pageCur"() {
                await this.getProductRank();
            },
            async "options.c3name"() {
                if (this.options.pageCur === 1) {
                    await this.getProductRank();
                } else {
                    this.options.pageCur = 1
                }
            }
        }
    }
</script>

<style scoped>

</style>


