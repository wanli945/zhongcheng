...
export const overAllData = async (that) => {
    ...
    // 获取 整体看板数据
    try {
        objData = (await that.$axios.post("terminal_showdistribution", {}, {
            params: that.options,
            showLoading: false
        })).data;
    } catch (e) {
...
export const getPolyline = async (that) => {
    ...
    try {
        const {data} = await that.$axios.post("terminal_showgraph", {}, {
            params: that.options,
            showLoading: false
        });
        // console.log(data)
...
export const getCityRank = async (that) => {
    let dataArr = [];
    try {
        const {data} = await that.$axios.post("terminal_showterrain", {}, {
            params: that.options,
            showLoading: false
        })
        // console.log(data)
...
export const getRank = async (that) => {
    let dataArr = [];
    try {
        const {data} = await that.$axios.post("terminal_showperformance", {}, {
            params: that.options,
            showLoading: false
        })
        data.forEach(value => {
...
export const getService = async (that) => {
    ...
    let total_ = 0;
    try {
        const {data, total} = await that.$axios.post("terminal_showQuota", {}, {
            params: that.options,
            showLoading: false,
        });
        // console.log(data)
...
export const getProduct = async (that) => {
    ...
    let total_ = 0;
    try {
        const {data, total} = await that.$axios.post("terminal_showProd", {}, {
            params: that.options,
            showLoading: false
        });
        data.forEach(value => {
